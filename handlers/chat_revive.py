...
async def update_role_mentionable(guild: discord.Guild, mentionable: bool):
    ...
    try:
        ...
        print(f"[CHAT_REVIVE] Role set to {state}")
    except discord.Forbidden:
        print("[CHAT_REVIVE] Missing permissions to edit role")
    except discord.HTTPException as e:
        print(f"[CHAT_REVIVE] Failed to edit role: {e}")
...
async def background_loop(client: discord.Client):
    ...
    while True:
        try:
            data = _load_data()
            now = time.time()
            elapsed = now - data["last_ping_time"]
            ...
            print(f"[CHAT_REVIVE] Background loop error: {e}")
...
async def handle_message(message: discord.Message) -> bool:
    ...
    if elapsed >= COOLDOWN_SECONDS:
        ...
        print(f"[CHAT_REVIVE] Ping used — {COOLDOWN_SECONDS/3600:.0f}h cooldown started")
    ...
    if warning_elapsed < WARNING_COOLDOWN_SECONDS:
        print("[CHAT_REVIVE] Warning suppressed (30min cooldown)")
        return True
    ...
    try:
        ...
        print(f"[CHAT_REVIVE] Warning sent to #{message.channel.name}")
        data["last_warning_time"] = now
        _save_data(data)
    except discord.Forbidden:
        print("[CHAT_REVIVE] Missing permissions to send warning")
